...
import mundo.Computador.Operativo;
...
public class Main {
    ...
    Camisa.Marca marca = null; 

    Camisa.Marca[] marcas = Camisa.Marca.values();
    for (int i = 0; i < marcas.length; i++) {
        System.out.println("[" + (i + 1) + "] " + marcas[i]);
    }

    int indice = Integer.parseInt(br.readLine());
    marca = marcas[indice - 1];
    ...
    Computador.Operativo[] sist = Computador.Operativo.values();
    ...
    int indice = Integer.parseInt(br.readLine());
    Computador.Operativo operativo = sist[indice - 1];
    ...
    Producto computador = new Computador(gama, precio, operativo);
